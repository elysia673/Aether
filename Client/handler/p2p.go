...
func (pm *p2pManager) runSource(sess *p2pSession) {
	...
	relayURL := fmt.Sprintf("%s://%s:9909/p2p-relay?session=%s&token=%s&role=source",
		scheme, sess.serverHost, sess.id, sess.token)
...
func (pm *p2pManager) runTarget(sess *p2pSession) {
	...
	relayURL := fmt.Sprintf("%s://%s:9909/p2p-relay?session=%s&token=%s&role=target",
		scheme, sess.serverHost, sess.id, sess.token)
