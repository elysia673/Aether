...
func (h *APIHandler) StartTCPProxy(port int, bindAddr string, table *manager.ClientTable, token string) {
	...
	log.Printf("TCP proxy listening on :%d for %s", port, clientID)
...
func handleTCPConnection(conn net.Conn, key string, token string, table *manager.ClientTable) {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("panic in handleTCPConnection: %v", r)
		}
	}()

	remoteAddr := conn.RemoteAddr().String()
	log.Printf("New TCP connection from %s for key %s", remoteAddr, key)
	...
	if proto.IsTunnelConn(br) {
		...
		if err == nil && receivedToken == token {
			log.Printf("Tunnel connection established for %s from %s, creating multiplexer", key, remoteAddr)
		...
		if err != nil {
			...
			log.Printf("Token mismatch from %s", remoteAddr)
		}
		conn.Close()
		return
	}
	...
	conn.SetReadDeadline(time.Time{})
	log.Printf("Public connection from %s for %s, getting multiplexer", remoteAddr, key)
...
func handleChannel(publicConn net.Conn, channel *mux.Channel, key string) {
	...
	wg.Wait()
	log.Printf("Channel %d closed for %s", channel.ID, key)
}
